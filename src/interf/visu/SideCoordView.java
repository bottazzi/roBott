...
/*
private BranchGroup createRobotReference(Programa p, Canvas3D c) //BranchGroup
{
  // Create the root of the branch graph
  ...
  //t.set(1.0f , new Vector3d(-(p.RobotOffset.Coord_X + p.WorkSpaceOffset.Coord_X)/scale, -(p.RobotOffset.Coord_Y + p.WorkSpaceOffset.Coord_Y)/scale, -(p.RobotOffset.Coord_Z + p.WorkSpaceOffset.Coord_Z)/scale)); //recua o rob  em rela  o a area de trabalho usando a pe a como refer ncia
  ...
  objTransGraphRef.setCapability(TransformGroup.ALLOW_TRANSFORM_WRITE);
	...
	objTransGraphRef.setCapability(TransformGroup.ENABLE_PICK_REPORTING);
       ...
       Sphere Robotcenter = new Sphere(0.02f);// Centro do BranchGroup root(centro do mundo, posicao do centro da base do rob )

       Appearance appRobotcenter = new Appearance();
       ColoringAttributes colorAttrsRobotcenter = new ColoringAttributes();
       colorAttrsRobotcenter.setColor(0.0f, 0.8f, 0.8f);//cor centro do mundo
       appRobotcenter.setColoringAttributes(colorAttrsRobotcenter);
       Robotcenter.setAppearance(appRobotcenter);
       
       objTransGraphRef.addChild(Robotcenter);
...
t2.print("Tempo a desenhar sceneGraphSelRef");
  ...
  viewingPlatform.setNominalViewingTransform();
        ...
        //viewingPlatform.setViewPlatformBehavior(null);
/*
